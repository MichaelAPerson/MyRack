# MyRack Server Installer - PowerShell Version
# Requires: Administrator privileges, Node.js, npm
...
if (-NOT ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) {
    Handle-Error "This script requires administrator privileges. Please run PowerShell as Administrator."
}
...
Write-ColorOutput "[*] Installing create-react-app..." "Yellow"
try {
    npm install -g create-react-app
    Write-ColorOutput "[*] create-react-app installed successfully" "Green"
} catch {
    Handle-Error "Failed to install create-react-app"
}

# Set app directory
$APP_DIR = "$env:USERPROFILE\myrack-dashboard"

# Remove existing directory if it exists
if (Test-Path $APP_DIR) {
    Write-ColorOutput "[*] Cleaning up existing directory: $APP_DIR" "Yellow"
    Remove-Item -Path $APP_DIR -Recurse -Force
}

# Create React app
Write-ColorOutput "[*] Creating React app in $APP_DIR..." "Yellow"
try {
    npx create-react-app $APP_DIR
    Write-ColorOutput "[*] React app created successfully" "Green"
} catch {
    Handle-Error "create-react-app failed"
}
...
try {
    ...
    Write-ColorOutput "[!] Warning: npm had warnings during dependency install." "Yellow"
}

# Update index.css
Write-ColorOutput "[*] Updating index.css..." "Yellow"
$indexCssContent = @'
...
$indexCssContent | Out-File -FilePath "src\index.css" -Encoding UTF8

# Write App.css
Write-ColorOutput "[*] Writing App.css..." "Yellow"
$appCssContent = @'
/* Reset and basics */
* {
  margin: 0;
...
$appCssContent | Out-File -FilePath "src\App.css" -Encoding UTF8

# Replace App.js
Write-ColorOutput "[*] Replacing App.js..." "Yellow"
$appJsContent = @'
...
$appJsContent | Out-File -FilePath "src\App.js" -Encoding UTF8

# Create a simple batch file to start the service (Windows equivalent of systemd)
Write-ColorOutput "[*] Creating startup batch file..." "Yellow"
$batchContent = @"
@echo off
cd /d "$APP_DIR"
echo Starting MyRack Dashboard...
npm start
"@

$batchContent | Out-File -FilePath "$APP_DIR\start-myrack.bat" -Encoding ASCII

# Create a VBS script to run the batch file hidden
$vbsContent = @"
CreateObject("Wscript.Shell").Run """$APP_DIR\start-myrack.bat""", 0, False
"@

$vbsContent | Out-File -FilePath "$APP_DIR\start-myrack-hidden.vbs" -Encoding ASCII

# Create a scheduled task for startup (Windows equivalent of systemd enable)
Write-ColorOutput "[*] Creating scheduled task for startup..." "Yellow"
try {
    $taskName = "MyRack-Dashboard"
    ...
    $action = New-ScheduledTaskAction -Execute "wscript.exe" -Argument "`"$APP_DIR\start-myrack-hidden.vbs`""
    $trigger = New-ScheduledTaskTrigger -AtStartup
    $settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable
    $principal = New-ScheduledTaskPrincipal -UserId $env:USERNAME -LogonType Interactive -RunLevel Limited
    
    Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Description "MyRack Dashboard Auto-Start"
    
    Write-ColorOutput "[*] Scheduled task created successfully" "Green"
} catch {
    Write-ColorOutput "[!] Warning: Could not create scheduled task. You may need to start the app manually." "Yellow"
}

# Start the application
Write-ColorOutput "[*] Starting MyRack Dashboard..." "Yellow"
try {
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c", "cd /d `"$APP_DIR`" && npm start" -WindowStyle Hidden
    Write-ColorOutput "[*] MyRack Dashboard started successfully" "Green"
} catch {
    Write-ColorOutput "[!] Warning: Could not start the application automatically. You can start it manually by running start-myrack.bat" "Yellow"
}
...
Write-ColorOutput "`nInstallation complete! The application should start automatically." "Green"
